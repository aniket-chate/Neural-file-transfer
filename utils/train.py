...
def parse_arguments():
    ...
    parser.add_argument('--content_dir', type=str, default="D:/Neural File Transfer/content_data", help="Location of content dataset")
    parser.add_argument('--style_dir', type=str, default="D:/Neural File Transfer/style_data", help ="Location of style dataset")
...
def main():
    ...
    content_dataloder = DataLoader(content_dataset, batch_size = args.batch_size, shuffle=True, pin_memory=True, drop_last=True)  
    style_dataloder = DataLoader(style_dataset, batch_size = args.batch_size, shuffle=True, pin_memory=True, drop_last=True)
    ...
    for epoch in range(args.epochs):
        progress_bar = tqdm(zip(content_dataloder, style_dataloder),
                            total=min(len(content_dataloder), len(style_dataloder)))
        ...
        for content_batch, style_batch in progress_bar:
            ...
            progress_bar.set_description(f'loss: {loss.item():4f}, content loss: {loss_c.item():4f}, style_loss: {loss_s.item():4f}')
        ...
        running_loss /= len(content_dataloder)
        running_closs /= len(content_dataloder)
        running_sloss /= len(content_dataloder) 
        
        if (epoch+1) % args.log_interval == 0:
            tqdm.write(f'Iter {epoch+1}: loss: {running_sloss:4f}, content loss: {running_closs:4f} , style_loss: {running_sloss:4f}') 
